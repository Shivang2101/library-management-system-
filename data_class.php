...
class data extends db {
    ...
    function addbook($bookpic, $bookname, $bookaudor, $bookpub, $branch, $bookprice, $bookquantity) {
       ...
       $q="INSERT INTO book (id,bookpic,bookname, bookaudor, bookpub, branch, bookprice,bookquantity,bookava,bookrent)VALUES('','$bookpic', '$bookname', '$bookaudor', '$bookpub', '$branch', '$bookprice', '$bookquantity','$bookquantity',0)";
    ...
    function getissuebook($userloginid) {
        ...
        // Select issued books for the user
        $query = "SELECT * FROM issuebook WHERE userid='$userloginid'";
        ...
        foreach ($recordSet->fetchAll() as $row) {
            ...
            if ($issuereturn < $currentDate) {
                ...
                $datetime1 = new DateTime($issuereturn);
                $datetime2 = new DateTime($currentDate);
                $interval = $datetime1->diff($datetime2);
                $daysOverdue = $interval->format('%a');
    
                // Calculate the fine
                $newfine = $fine + ($daysOverdue * 10); // Fine increases by Rs. 10 for each overdue day
            }
        }
    
        // Update fine in the database
        if ($newfine > 0) {
            $query = "UPDATE issuebook SET fine='$newfine' WHERE userid='$userloginid'";
            if ($this->connection->exec($query)) {
                // Fine updated successfully, fetch updated data
                $query = "SELECT * FROM issuebook WHERE userid='$userloginid'";
                $data = $this->connection->query($query);
                return $data;
            } else {
                // Error updating fine, return existing data
                $query = "SELECT * FROM issuebook WHERE userid='$userloginid'";
                $data = $this->connection->query($query);
                return $data;
            }
        } else {
            // No fine, return existing data
            $query = "SELECT * FROM issuebook WHERE userid='$userloginid'";
            $data = $this->connection->query($query);
            return $data;
        }
